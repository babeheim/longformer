...
update_id_changes <- function(new_change, changelog, verbose = TRUE) {
  ...
  if (new_change$old_id %in% changelog$active_id) warning("id ", new_change$old_id, " will be retired but was already mapped as an active_id; is the right individual being updated?")
